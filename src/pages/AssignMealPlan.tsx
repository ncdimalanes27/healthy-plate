...
export default function AssignMealPlan() {
  const { currentUser, getAllPatients, getProfile, assignMealPlan, getAssignedPlansForPatient, getMealPlans } = useStore();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={label} className="bg-white rounded-xl px-3 py-2">
                    ...
                    <p className="text-sm font-semibold text-blue-800 mt-0.5">{val}</p>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <p className="text-sm font-semibold text-gray-700 mb-4">Generate & Assign 7-Day Meal Plan</p>

            <div className="space-y-3 mb-4">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">
                  Custom calories (optional — default is patient's calculated target)
                </label>
                <input
                  type="number"
                  value={customCals}
            ...
            >
              {assigned ? <CheckCircle className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
            ...
            {assignedPlans.length === 0 ? (
              ...
              <div className="space-y-2">
                {assignedPlans.map((plan) => (
                  <div key={plan.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
                    <button
                      onClick={() => setExpandedPatient(expandedPatient === plan.id ? null : plan.id)}
                      className="w-full flex items-center justify-between px-5 py-4 hover:bg-gray-50 transition-colors"
                    >
                      <div className="text-left">
                        <p className="font-semibold text-gray-800 text-sm">{plan.mealPlanName}</p>
                        <p className="text-xs text-gray-400">
                          Assigned {new Date(plan.assignedAt).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })} by {plan.dieticianName}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        {expandedPatient === plan.id ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
                      </div>
                    </button>
                    {expandedPatient === plan.id && plan.note && (
                      <div className="px-5 pb-4 border-t border-gray-50">
                        <p className="text-xs text-gray-500 mt-3 font-medium mb-1">Dietician note:</p>
