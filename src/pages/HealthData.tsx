...
import { getAllFoods, searchFoods, getFoodsByCategory, getTodayLog, getMealEntries, insertMealEntry, upsertDailyLog } from '../lib/supabaseService';
import { foodCategories, budgetFoods } from '../data/foods';
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Search from our food database and log your meals.</p>
      </div>
      ...
      <div className="flex gap-2 flex-wrap mb-5">
        <button onClick={() => handleCategoryClick('All')}
          className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
            activeCategory === 'All' ? 'bg-green-600 text-white border-green-600' : 'bg-white border-gray-200 text-gray-600 hover:border-green-300'
          }`}>
          🍽️ All
        </button>
        {foodCategories.map((cat) => (
          <button key={cat} onClick={() => handleCategoryClick(cat)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
              activeCategory === cat
                ? cat === 'Budget Meals' ? 'bg-amber-500 text-white border-amber-500' : 'bg-green-600 text-white border-green-600'
      ...
      {activeCategory === 'Budget Meals' && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 mb-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">Masustansya at abot-kaya para sa lahat!</p>
          </div>
        </div>
      )}
      ...
      <p className="text-xs text-gray-400 mb-3">
        {loading ? 'Loading foods...' : `Showing ${displayed.length} of ${foods.length} results${activeCategory !== 'All' ? ` in ${activeCategory}` : ''}${search ? ` for "${search}"` : ''}`}
      </p>
              ...
              <div key={food.id} className="bg-white border border-gray-100 rounded-2xl px-4 py-3 flex items-center justify-between hover:shadow-sm transition-shadow">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{food.category} · {food.serving_size}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
